...
class KGDataset(object):
    def __init__(self, dataset, inter_file, kg_data_path, output_path, hop):
        ...
        self.kg_file_list = [
            os.path.join(kg_data_path, "hop" + str(i + 1) + ".kg") for i in range(3)
        ]
        ...
        self.seed_entities, self.seed_link, self.item_field = self.get_seed_entities(
            inter_file, os.path.join(kg_data_path, "link.kg")
        )
        self.selected_relations = self.get_selected_relations(
            os.path.join(kg_data_path, "relation.kg")
        )
    ...
    def generate_knowledge(self):
        ...
        for i in range(self.hop):
            ...
            seed_entities, temp_triples = extract_hop_graph(
                kg_file, self.selected_relations, seed_entities, history_entities
            )
            hop_triples += temp_triples
        extra_triples = extract_extra_graph(
            extra_kg_file, self.selected_relations, history_entities
        )
