...
class CosmeticsDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        self.output_item_file = os.path.join(
            self.output_path, f"{self.dataset_name}.item"
        ).replace("\\", "/")
        self.output_inter_file = os.path.join(
            self.output_path, f"{self.dataset_name}.inter"
        ).replace("\\", "/")
    ...
    def convert_inter(self):
        ...
        self.df[self.inter_fields].to_csv(
            self.output_inter_file, sep=self.sep, index=False
        )

    def convert_item(self):
        self._read_data_from_files()
        self.log.info(f"Writing to item file")
        self.df[self.item_fields].to_csv(
            self.output_item_file, sep=self.sep, index=False
        )
