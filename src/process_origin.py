...
if __name__ == "__main__":
    ...
    ), "input_path can not be None, please specify the input_path"
    assert (
        args.output_path is not None
    ), "output_path can not be None, please specify the output_path"

    input_args = [args.input_path, args.output_path]
    dataset_class_name = dataset2class[args.dataset.lower()]
    dataset_class = getattr(
        importlib.import_module("src.extended_dataset"), dataset_class_name
    )
